...
export default function Modal({
  ...
  useEffect(() => {
    ...
    setSelected(
      dietaryconditions.filter((item) =>
        marker.tags.some((item2) => item.name === item2)
      )
    );
  ...
  const saveMarkerTags = useCallback(() => {
    ...
    geodatabase.restaurants
      .doc(markerRef)
      .set({ tags: selected.map((item) => item.name) }, { merge: true });
  }, [selected]);
              ...
              {dietaryconditions.map((condition, index) => (
                <Grid container item={true} key={index} xs={12} md={4} lg={4}>
                  <FormControlLabel
                    ...
                    control={
                      <CustomCheckbox
                        value={JSON.stringify({ id: condition.path })}
